...
window.addEventListener("resize", resizeSunPathCanvas);

function resizeSunPathCanvas() {
  ...
  // 🔄 Re-draw your diagram here
  // NOTE: Call your function with proper arguments
  // Example (assuming you have sunriseUTC, sunsetUTC, and cityNowDate stored globally):
  if (window._lastSunrise && window._lastSunset && window._lastCityDate) {
    drawAdvancedSunPath(window._lastSunrise, window._lastSunset, window._lastCityDate);
  }
}
...
navLinks.forEach(link => {
  link.addEventListener('click', (e) => {
    ...
    else if (targetText === "☀️ Sun Diagram") {
      document.querySelector('.sun-path-container').scrollIntoView({ behavior: 'smooth' });
    }
  });
});
